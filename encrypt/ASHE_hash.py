...
class FlasheCipher_Hash(object):
    ...
    def _multiprocessing_encrypt(self, value,first_random):
        ...
        ret = value + self.next_iter_encrypt_prepared['add'] - self.next_iter_encrypt_prepared['minus'] + first_random
        ...
        return ret
    # 加密操作
    def encrypt(self, plaintext,first_random):
        if self.hash_algorithm is not None:
            ...
            if not isinstance(plaintext, np.ndarray):
                ...
                if self.masking_scheme == "double":
                    return self._multiprocessing_encrypt(plaintext,first_random)
                else:
